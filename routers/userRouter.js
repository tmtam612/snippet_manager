    ...
    try {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            if (
                ...
                let errorField = [];
                errorField =
                    email === '' ? [...errorField, 'email'] : [...errorField];
                errorField =
                    user_name === ''
            ...
            if (status === '') {
                return res.send({
                    status: false,
                    errorMessage: 'Please check accept checkbox.',
                    errorField: ['status'],
                });
            }
            ...
            if (password !== confirm_password) {
                return res.send({
                    status: false,
                    errorMessage:
                        'Please enter the same twice for verification.',
                    errorField: ['password', 'confirm_password'],
                });
            }
            ...
            if (exisingUser) {
                return res.send({
                    status: false,
                    errorMessage: 'Account has existed.',
                    errorField: ['email', 'user_name'],
                });
            }
            ...
            //         const newpath =
            //             './uploads/' + savedUser._id + '/' + files.image.name;
            ...
            //     return res.send({
            //         status: false,
            //         errorMessage: 'There is error in server',
            //     });
            // }
        });
    } catch (err) {
